...
export class Workflow extends Component {
  ...
  displayTabs = () => {
    ...
    for (let i = 0; i < data.length; i++) {
      let row = data[i];
      retData.push(
        <li key={`tab-${i}`} className={i === activeIndex ? 'active' : ''} onClick={(e) => this.navigateTab(i)}>
          <a>{row.tabTitle}</a>
        </li>
      );
    }
        ...
        <div className="workflow-tab-content">
          ...
          <div className="d-block pre-next-buttons">
            <button
              ...
              onClick={(e) => this.navigateTab(activeIndex - 1)}
            >
              Previous
            </button>
            ...
            {activeIndex === data.length - 1 && (
              <button
                className={`blue-button float-right m-r-0 ${loading ? 'disable' : ''}`}
                onClick={this.onClickNext}
                disabled={loading}
              >
                Finish
