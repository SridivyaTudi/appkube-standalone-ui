...
class Tree extends React.Component {
  ...
  componentDidUpdate = (prevProps, prevState) => {
    if (
      JSON.stringify(prevProps.valueForTree) !==
      JSON.stringify(this.props.valueForTree)
    ) {
      this.setState({
        treeData: this.props.valueForTree,
      });
  ...
  renderTree = (folder, indexArr) => {
        ...
        subFolderJSX.push(
          <ul>
            {subFolder.subData == undefined && (
              <li>
                {subFolder.name} <span>{subFolder.type}</span>
                {subFolder.type == "Array" && (
                  <a href="#">{subFolder.length}</a>
                )}
              </li>
            )}
            {subFolder.subData && this.renderTree(subFolder, subIndexArr)}
          </ul>
        );
      }
    }
    retData.push(
      <li>
        {folder.subData != undefined && (
          <i
            className={
              folder.isOpened != true ? "fa fa-caret-right" : "fa fa-caret-down"
            }
            onClick={() => this.onClickOpenSubTreeArr([...indexArr])}
          ></i>
        )}
        {folder.name}
        <span>{folder.type}</span>
        {folder.type == "Array" && <a href="#">{folder.length}</a>}
        {folder.isOpened == true && <Collapse>{subFolderJSX}</Collapse>}
      </li>
    );
    return retData;
  };
